...
for i in range(deconv_layers):
    net = tf.layers.conv2d(net, filter_size, deconv_filters[i], strides=strides, padding='valid',
...
# gen_loss_op = tf.log(
#     tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=gen_x_, logits=gen_x, name="gen_loss"),
...
train_op = tf.train.AdamOptimizer(learning_rate=0.01, name="optimizer").minimize(class_loss_op, name="minimize")
...
for epoch in range(epochs):
    for train_step in range(num_batch):
        ...
        _, class_loss, summary, accuracy = sess.run(
            [train_op, class_loss_op, merged, accuracy_op],
            feed_dict={x: batch_xs, y_: batch_ys})
        now = datetime.now().strftime('%m/%d %H:%M:%S')
        print("[%s TRAIN %d epoch, %d/%d step] class_loss: %f, accuracy: %f" % (
            now, epoch, train_step, num_batch, class_loss, accuracy))
    ...
    for test_step in range(num_test_batch):
        ...
        accuracy, class_loss, summary = sess.run(
            [accuracy_op, class_loss_op, merged], feed_dict={x: test_xs, y_: test_ys})
        total_accuracy += accuracy
        now = datetime.now().strftime('%m/%d %H:%M:%S')
        print("[%s TEST %d epoch, %d/%d step] accuracy: %f, class_loss: %f" % (
            now, epoch, test_step, num_test_batch, accuracy, class_loss))
