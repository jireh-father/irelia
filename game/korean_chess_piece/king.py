...
def get_actions(state_map, x, y):
    ...
    if ((y == 7 and x == 3) or (y == 8 and x == 4)) and \
        ...
        # and not kcu.is_losing_way(state_map, x, y, x + 1, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x + 1, 'to_y': y + 1, 'direction': kcu.DIAGONAL_BACKWARD_RIGHT1, 'step': 1,
             'piece_type': piece_type})
    ...
    if ((y == 7 and x == 5) or (y == 8 and x == 4)) and \
        ...
        # and not kcu.is_losing_way(state_map, x, y, x - 1, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x - 1, 'to_y': y + 1, 'direction': kcu.DIAGONAL_BACKWARD_LEFT1, 'step': 1,
             'piece_type': piece_type})
    ...
    if y in (8, 9) and not kcu.is_our_side(state_map, x, y - 1, side):
        ...
        action_list.append(
            {'x': x, 'y': y, 'to_x': x, 'to_y': y - 1, 'direction': kcu.FORWARD, 'step': 1,
             'piece_type': piece_type})
    ...
    if y in (7, 8) and not kcu.is_our_side(state_map, x, y + 1, side):
        # and   not kcu.is_losing_way(state_map, x, y, x, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x, 'to_y': y + 1, 'direction': kcu.BACKWARD, 'step': 1,
             'piece_type': piece_type})
