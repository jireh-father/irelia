...
gen_y_ = tf.image.resize_images(inputs, [87, 87])
gen_y_ = tf.reshape(gen_y_, [-1, 87 * 87])
...
gen_image = tf.nn.sigmoid(net, name="gen_sigmoid")
gen_image = tf.reshape(gen_image, [-1, 87 * 87], "gen_image_reshape")
net = tf.reshape(net, [-1, 87 * 87], name="reshape_fc")
...
gen_loss_op = tf.log(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=gen_y_, logits=gen_image)))
...
for epoch in range(epochs):
    for train_step in range(num_batch):
        ...
        _, class_loss, gen_loss, total_loss = sess.run(
            [train_op, class_loss_op, gen_loss_op, total_loss_op], feed_dict={x: batch_xs, y_: batch_ys})
    ...
    for test_step in range(num_test_batch):
        ...
        accuracy, class_loss, gen_loss, total_loss = sess.run(
            [accuracy_op, class_loss_op, gen_loss_op, total_loss_op], feed_dict={x: test_xs, y_: test_ys})
