...
def get_actions(state_map, x, y, color=Constant.BLUE, is_hash_map=False):
    ...
    if ((y == 9 and x == 3) or (y == 8 and x == 4)) and \
      not game.is_our_side(state_map, x + 1, y - 1, side):
        ...
        if is_hash_map:
            hash_list["%d_%d_%d_%d" % (x, y, x + 1, y - 1)] = True
        else:
            action_list.append(
                {'x': x, 'y': y, 'to_x': x + 1, 'to_y': y - 1, 'direction': Constant.DIAGONAL_FORWARD_RIGHT1, 'step': 1,
    ...
    if ((y == 7 and x == 3) or (y == 8 and x == 4)) and \
      not game.is_our_side(state_map, x + 1, y + 1, side):
        # and not util.is_losing_way(state_map, x, y, x + 1, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x + 1, 'to_y': y + 1, 'direction': Constant.DIAGONAL_BACKWARD_RIGHT1, 'step': 1,
             'piece_type': piece_type})
    ...
    if ((y == 7 and x == 5) or (y == 8 and x == 4)) and \
      not game.is_our_side(state_map, x - 1, y + 1, side):
        # and not util.is_losing_way(state_map, x, y, x - 1, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x - 1, 'to_y': y + 1, 'direction': Constant.DIAGONAL_BACKWARD_LEFT1, 'step': 1,
             'piece_type': piece_type})
    ...
    if y in (8, 9) and not game.is_our_side(state_map, x, y - 1, side):
        ...
        action_list.append(
            {'x': x, 'y': y, 'to_x': x, 'to_y': y - 1, 'direction': Constant.FORWARD, 'step': 1,
             'piece_type': piece_type})
    ...
    if x in (3, 4) and not game.is_our_side(state_map, x + 1, y, side):
        ...
        action_list.append(
            {'x': x, 'y': y, 'to_x': x + 1, 'to_y': y, 'direction': Constant.RIGHT, 'step': 1,
             'piece_type': piece_type})
    ...
    if x in (4, 5) and not game.is_our_side(state_map, x - 1, y, side):
        # and   not util.is_losing_way(state_map, x, y, x - 1, y, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x - 1, 'to_y': y, 'direction': Constant.LEFT, 'step': 1, 'piece_type': piece_type})

    # 후진 길 체크
    if y in (7, 8) and not game.is_our_side(state_map, x, y + 1, side):
        # and   not util.is_losing_way(state_map, x, y, x, y + 1, side):
        action_list.append(
            {'x': x, 'y': y, 'to_x': x, 'to_y': y + 1, 'direction': Constant.BACKWARD, 'step': 1,
             'piece_type': piece_type})
